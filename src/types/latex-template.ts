...
export class LatexLoopItemData {
    ...
    constructor(header: string, loop: LatexArgItemData, footer: string, afterEach?: string) {
        this.header = header;
        this.loop = loop;
        this.footer = footer;
...
export class LatexTemplateData {
    ...
    sections: (LatexSectionData | LatexSkillData | LatexInformationData | LatexLanguageData)[];
...
export class LatexSection {
    ...
    getLatexString(resume: Resume): string {
        const sectionExpectedArgs = getLatexExpectedArgs()[this.data.type] as ExpectedArg<Section>[];
                ...
                if (this.data.bulletPoints && Array.isArray(section.content)) {
                    ...
                    for (const bullet of section.content) {
                        ...
                        if (bullet !== section.content[section.content.length - 1]) {
                            bulletPointsContent += this.data.bulletPoints.afterEach ? this.data.bulletPoints.afterEach : '';
                        }
                    }
...
export class LatexLanguage {
    ...
    getLatexString(resume: Resume): string {
        ...
        if (languages) {
            let processedContent = this.data.header + '\n';
            const loopItem = new LatexArgItem(this.data.loop);
            const languageExpectedArgs = getLatexExpectedArgs()['languages'] as ExpectedArg<LanguageLevel>[];
...
export class LatexInformation {
    ...
    getLatexString(resume: Resume): string {
        ...
        const infoExpectedArgs = getLatexExpectedArgs()['information'] as ExpectedArg<Resume>[];
...
export class LatexTemplate {
    ...
    generateLatex(resume: Resume): string {
        ...
        const sortedSections = [...this.data.sections].sort((a, b) => a.order - b.order);
...
export function getLatexExpectedArgs(): Record<string, (ExpectedArg<Section> | ExpectedArg<Resume> | ExpectedArg<LanguageLevel>)[]> {
    return {
